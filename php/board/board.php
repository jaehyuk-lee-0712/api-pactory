...
// 검색어 처리
$searchKeyword = isset($_GET['searchKeyword']) ? $connection->real_escape_string($_GET['searchKeyword']) : '';
// 게시물 가져오기 쿼리
                ...
                <div class="board__inner">
                    ...
                    <div class="board__pages">
                        <ul>
                            ...
                            // 처음으로/ 이전
                            if ($page > 1) {
                                echo "<li class='first'><a href='board.php?page=1&searchKeyword=$searchKeyword'>처음으로</a></li>";
                                echo "<li class='first'><a href='board.php?page={$beforePage}&searchKeyword=$searchKeyword'>이전</a></li>";
                            }
                            // 페이지 번호
                            for ($i = $startPage; $i <= $endPage; $i++) {
                                $active = ($i == $page) ? "active" : "";
                                echo "<li><a class='{$active}' href='board.php?page={$i}&searchKeyword=$searchKeyword'>{$i}</a></li>";
                            }
                            // 다음/ 마지막으로
                            if ($page < $boardTotalCnt) {
                                echo "<li class='first'><a href='board.php?page={$nextPage}&searchKeyword=$searchKeyword'>다음</a></li>";
                                echo "<li class='end'><a href='board.php?page={$boardTotalCnt}&searchKeyword=$searchKeyword'>마지막으로</a></li>";
                            }
                            ?>
                        </ul>
                    </div>
